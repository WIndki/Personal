...
void print() 打印链表
*/
// void MergeList(LinkedList<T> &L1,LinkedList<T> &L2,LinkedList<T>
// &L3);//合并两个有序链表
template <class T>
...
void MergeList(LinkedList<int> &list1,
               LinkedList<int> &list2) {  // 合并两个有序链表
  int i = 0, j = 0;
  while (i < list1.getLength() && j < list2.getLength()) {
    if (list1.get(i) < list2.get(j)) {
      i++;
    } else {
      list1.insert(i, list2.get(j));
      j++;
    }
  }
  while (j < list2.getLength()) {
    list1.push_back(list2.get(j));
    j++;
  }
}
